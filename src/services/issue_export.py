...
def export_issues_to_github(github_url, gitlab_issues, github_issues, gitlab_max_id, github_max_id, github_hidden_max_id, placeholders_options):
    ...
    labels = github_get.read_labels(github_url)
    new_labels = get_not_included_labels(gitlab_issues, labels)

    # determine missing milestones
    milestones = github_get.read_milestones(github_url)
    ...
    for current_issue in modified_issues:
        update_github_issue(github_url, gitlab_issues[current_issue[0]])
    ...
    for index in range(github_hidden_max_id + 1, gitlab_max_id + 1):
        if index not in gitlab_issues:
            create_github_issue(github_url, {"title": "placeholder"})
            new_placeholders.append(index)
            if placeholders_options[0] == True:
                update_github_issue(github_url, {"id": index, "title": "placeholder", "state": "closed"})
        else:
            create_github_issue(github_url, gitlab_issues[index])
            ...
            if gitlab_issues[index].state == "closed":
                update_github_issue(github_url, gitlab_issues[index])
...
def export_comments_to_github(github_url, gitlab_comments, github_comments, delete_missing_comments):
    updated_comments = [0, []]
    deleted_comments = [0, []]    
    ...
    for key in gitlab_comments.keys():
        if key not in github_comments: # then create all comments
            for current_comment in gitlab_comments[key]:
                create_github_comment(github_url, key, current_comment[0])
            ...
            while(len(gitlab_comments[key]) != 0):
                ...
                if not contained:
                    create_github_comment(github_url, key, gitlab_comments[key][0][0])
                    updated_comments[0] += 1
                    if key not in updated_comments[1]:
                        updated_comments[1].append(int(key))
    ...
    if delete_missing_comments:
        for key, remaining_comments in github_comments.items():
            while(len(remaining_comments) != 0):
                delete_github_comment(github_url, remaining_comments[0][1])
